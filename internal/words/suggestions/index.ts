...
export const NewListFromFile = (data: string): ListResult => {
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        if (line.length > MAX_SUGGESTION_LENGTH) return CreateError(i, line, 'Line too long');
    }
...
const CreateError = (lineNumber: number, suggestion: string, err: string): ListResult => {
    return {
        list: undefined,
