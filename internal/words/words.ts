...
export const NewListFromFile = (data: string): ListResult => {
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        try {
            if (line.startsWith('#')) { // Definition line that doesn't contain words
                const colonIndex = line.indexOf(':');
                if (colonIndex === -1) return CreateError(i, 'N/A', 'Error parsing definition: no colon specified');
            ...
            for (const roleString of roleArray) { 
                ...
                if (roleString.length !== final.length && final.length === 0) return CreateError(i, location, `Error parsing roles: role cannot be empty`);
                roles.push(final);
            }
...
const CreateError = (lineNumber: number, locationName: string, err: string): ListResult => {
    return {
        listName: undefined,
