...
import { JoinRequest, RoomRequest, ValidJoinRequest, ValidRoomRequest } from '../../protocol';
...
router.post('/api/room', async function(req, res) {
    const body = req.body;
    const data = new RoomRequest(body.nickname, body.roomName, body.roomPass, body.create);
    ...
    try { const valid = ValidRoomRequest(data); if (!valid[1]) { res.status(400).send(JSON.stringify({error: valid[0], mistake: valid[2]})); return; } } catch { return; }
    ...
    if (data.create) {
        ...
        res.status(200).send(JSON.stringify({roomID: room[0].GetID(), err: undefined}));
    }
    else {
        var roomSearch = await server.FindRoom(data.roomName);
        if (roomSearch == undefined || data.roomPass !== roomSearch.GetPassword()) { 
            res.status(404).send(`{"error":"Room not found or password is incorrect.","mistake":3}`); 
            return; 
        }
        ...
        res.status(200).send(JSON.stringify({roomID: roomSearch.GetID(), err: undefined}));
    }
});

router.post('/api/join', async function(req, res) { // For direct users that only provide a room ID
    const body = req.body;
    const data = new JoinRequest(body.nickname, body.roomID);
    if (!data.Sanitize()) return;

    try { const valid = ValidJoinRequest(data); if (!valid[1]) { res.status(400).send(JSON.stringify({error: valid[0], mistake: valid[2]})); return; } } catch { return; }

    var room = await server.FindRoomByID(data.roomID);
    if (!room) { res.status(400).send(`{"error":"Room not found."}`); return; } // This shouldn't happen since /api/exist is checked first
    if (room.HasStarted()) { res.status(403).send(`{"error":"Room has already started. Try joining later."}`); return; }
    if (room.GetClientCount() >= MAX_ROOM_MEMBER_COUNT) { res.status(403).send(`{"error":"Room is full."}`); return; }
