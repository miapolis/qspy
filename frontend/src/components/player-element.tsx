...
export default function PlayerElement ({name, discriminator, isHost, score, rank, onKick}:PlayerElementProps) {
    const host = React.useContext(UserContext);;
    const [hovered, setHovered] = React.useState<boolean>(false);
    const nameText = isHost ? 'nameText host' : 'nameText';
    return (
        <div className='playerElement' onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
            <div> 
