...
function PreStart ({ starting, playerCount, timerLength, packs, isHost, send }: PreStartProps) {
    const classes = useStyles();

    const [packDialogOpen, setPackDialogOpen] = React.useState(false);
    const changeTime = (add: boolean) => {
        if (add && timerLength + 30 <= 600) { send.changeTime(timerLength + 30); return; }
        else if (!add && timerLength - 30 >= 300) send.changeTime(timerLength - 30);
    }
            ...
            {isHost ? 
                <div>
                    <div className={classes.controlGroup}>
                        ...
                        <PackSelection isOpen={packDialogOpen} packs={packs} send={send} onClose={() => setPackDialogOpen(false)}/>
                        <div style={{
                            display: 'flex',
                            justifyContent: 'center',
                            flexFlow: 'row'
